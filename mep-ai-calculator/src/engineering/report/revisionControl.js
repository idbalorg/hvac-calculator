...
export const createRevisionHistory = ({ packageData, changedBy, changedAt, reason = "Initial design package" }) => {
  ...
  const revision = { revisionId: makeRevisionId(1), revisionNumber: 1, status: "DRAFT", createdAt: changedAt, createdBy: changedBy, changeReason: reason, affected: normalizeAffected(), approvalInvalidated: false, packageSnapshot: null };
  const snapshot = clone(packageData);
  snapshot.revision = { revisionId: revision.revisionId, revisionNumber: revision.revisionNumber, status: revision.status, createdAt: revision.createdAt, createdBy: revision.createdBy, changeReason: revision.changeReason, affected: revision.affected, approvalInvalidated: false };
...
export const createRevision = ({ history, packageData, changedBy, changedAt, reason, affected = {} }) => {
  ...
  const revisions = history.revisions.map((item) => item.revisionId === previous?.revisionId && wasApproved ? { ...item, status: "SUPERSEDED", supersededAt: changedAt, supersededBy: nextId } : item);
  ...
  if (wasApproved || nextPackage.engineeringApproval?.approved) {
    ...
    if (nextPackage.readiness) nextPackage.readiness.engineeringApprovalStatus = "PENDING";
  }
  ...
  const revision = { ...nextPackage.revision, packageSnapshot: immutableSnapshot(nextPackage) };
...
export const updateRevisionStatus = ({ history, revisionId, status }) => {
  ...
  return { ...history, revisions: history.revisions.map((item) => item.revisionId === revisionId ? { ...item, status } : item) };
};
