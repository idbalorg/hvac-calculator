...
export default function AirsideDesign() {
  ...
  const calculate = () => {
    ...
    try {
      ...
      if (projects.length) {
        ...
        current.result = { ...current.result, airside: { ...(current.result?.airside || {}), roomResults: [...(current.result?.airside?.roomResults || []).filter((item) => item.roomId !== roomResult.roomId), { roomId: roomResult.roomId, roomName: roomResult.roomName, inputs: { supplyDryBulbC: Number(supplyDryBulbC), leavingRHPercent: Number(leavingRH), outdoorAirflowM3s: Number(outdoorAirflowM3s) || 0 }, ...airside }] } };
  ...
  return <div className="container">
    ...
    <div className="grid calculator-grid">
      <div className="card"><div className="section-heading"><h3>Airside Design Inputs</h3><span>01</span></div>
        {!saved && <p className="form-note">Calculate and save a project on the main calculator first.</p>}
        {saved && <div className="input-grid"><div><label>Room</label><select value={selectedRoomId} onChange={(e) => setSelectedRoomId(e.target.value)}>{saved.rooms.map((room) => <option key={room.id} value={room.id}>{room.id} · {room.name || "Unnamed room"}</option>)}</select></div><Input label="Supply-air dry bulb" value={supplyDryBulbC} onChange={setSupplyDryBulbC} type="number" placeholder="°C" /><Input label="Coil leaving-air RH" value={leavingRH} onChange={setLeavingRH} type="number" min="0" max="100" placeholder="%" /><Input label="Outdoor airflow" value={outdoorAirflowM3s} onChange={setOutdoorAirflowM3s} type="number" min="0" placeholder="m³/s" /></div>}
        {error && <p className="error-message">{error}</p>}<button onClick={calculate} disabled={!saved}>Run Airside Calculation</button>
      </div>
      {result && <div className="results-stack">
        <div className="card"><div className="section-heading"><h3>Room Airside Summary</h3><span>02</span></div><Stat label="Room" value={`${result.roomResult.roomId} · ${result.roomResult.roomName}`} /><Stat label="Design sensible load" value={`${(result.roomResult.designLoad.sensibleW / 1000).toFixed(2)} kW`} /><Stat label="Design total load" value={`${(result.roomResult.designLoad.totalW / 1000).toFixed(2)} kW`} /><Stat label="Sensible heat ratio" value={result.airside.sensibleHeatRatio.toFixed(3)} /><Stat label="Required supply airflow" value={`${result.airside.airflow.airflowM3h.toFixed(0)} m³/h · ${result.airside.airflow.airflowLps.toFixed(1)} L/s`} /></div>
        ...
        <div className="card"><div className="section-heading"><h3>Cooling Coil</h3><span>04</span></div><Stat label="Entering enthalpy" value={`${result.airside.coil.enteringEnthalpyKJkg.toFixed(2)} kJ/kg`} /><Stat label="Leaving enthalpy" value={`${result.airside.coil.leavingEnthalpyKJkg.toFixed(2)} kJ/kg`} /><Stat label="Coil sensible load" value={`${(result.airside.coil.sensibleLoadW / 1000).toFixed(2)} kW`} /><Stat label="Coil latent load" value={`${(result.airside.coil.latentLoadW / 1000).toFixed(2)} kW`} /><Stat label="Coil total load" value={`${result.airside.coil.totalLoadKW.toFixed(2)} kW`} /></div>
        <div className="card"><p className="engineering-note"><b>Engineering basis:</b> Supply temperature, coil leaving RH and outdoor airflow are explicit design inputs. Confirm final supply-air condition, ventilation requirement, fan heat, duct heat gain and manufacturer coil selection during detailed design.</p></div>
      </div>}
    </div>
  </div>;
}
