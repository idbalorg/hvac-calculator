...
import { createEngineeringInputs, createEquipmentItem, ENGINEERING_INPUT_VERSION } from "../engineering/project/engineeringInputs.js";
...
import { applyReferenceToEngineeringInputs } from "../engineering/reference/referenceCalculationBridge.js";
...
import ReferenceTraceabilityPanel from "../components/ReferenceTraceabilityPanel.jsx";
...
export default function CoolingLoadCalculator() {
  ...
  const [appliedReferenceByRoom, setAppliedReferenceByRoom] = useState({});
  ...
  const removeRoom = (index) => {
    ...
    setAppliedReferenceByRoom((current) => { const next = { ...current }; delete next[removed.id]; return next; });
  };
  ...
  const applyReferenceInputs = (roomId) => {
    ...
    setEngineeringByRoom((current) => {
      ...
      const applied = applyReferenceToEngineeringInputs(base, resolved);
      const next = clone(applied.inputs);
    ...
    setAppliedReferenceByRoom((current) => ({ ...current, [roomId]: applyReferenceToEngineeringInputs(clone(engineeringByRoom[roomId] || DEFAULT_INPUTS), resolved) }));
  ...
  const calculate = () => {
    ...
    try {
      ...
      const baseDesignConditions = buildProjectDesignConditions({ outdoorConditionId: designConditionId, coolingPercentile, indoorDryBulbC: Number(indoorDryBulbC), indoorRelativeHumidityPercent: Number(indoorRelativeHumidityPercent), outdoorRelativeHumidityPercent: null });
      const outdoorRelativeHumidityPercent = deriveOutdoorRelativeHumidity(baseDesignConditions.selectedCoolingCondition);
      const designConditions = buildProjectDesignConditions({ outdoorConditionId: designConditionId, coolingPercentile, indoorDryBulbC: Number(indoorDryBulbC), indoorRelativeHumidityPercent: Number(indoorRelativeHumidityPercent), outdoorRelativeHumidityPercent });
      const engineering = buildEngineeringInputs({ rooms: engineeringRooms, engineeringByRoom, designConditions, ventilationRequired: criteria.ventilation });
      ...
      const referenceTraceByRoom = buildReferenceTraceByRoom({ roomIds: engineeringRooms.map((room) => room.id), engineeringByRoom: engineering, appliedReferenceByRoom });
      setResult({ rooms: builtRooms, summary: projectSummary, loads, decision, designConditions, engineering, referenceByRoom: clone(referenceByRoom), referenceTraceByRoom });
  ...
  const saveProject = () => {
    ...
    localStorage.setItem("hvac-projects", JSON.stringify([...projects, { rooms, engineeringByRoom, referenceByRoom, appliedReferenceByRoom, criteria, zoning, designMarginPercent, designConditionId, coolingPercentile, indoorDryBulbC, indoorRelativeHumidityPercent, result, date: new Date().toISOString(), inputModelVersion: ENGINEERING_INPUT_VERSION }]));
  };

  return <div className="container">
    <div className="page-header"><div><p className="eyebrow">ENGINEERING WORKFLOW · STAGE 36</p><h1 className="title">HVAC Design Calculator</h1><p className="subtitle">Reference-driven project and room inputs connected to the component cooling-load engine.</p></div><span className="version-badge">Input Model {ENGINEERING_INPUT_VERSION}</span></div>
    <div className="grid calculator-grid">
      <div className="card">
        ...
        <p className="form-note">When dedicated ventilation is checked, room Rp/Ra values are included in the cooling load. If both remain zero, a generic 8 L/s-person + 0.3 L/s-m² starter basis is used. This is not an ASHRAE 62.1 compliance determination.</p>
      ...
      {result && <div className="results-stack">
        <div className="card"><div className="section-heading"><h3>Design Conditions</h3><span>04</span></div><div className="stat"><span>Location</span><b>{result.designConditions.outdoor.location}</b></div><div className="stat"><span>Source</span><b>ASHRAE {result.designConditions.outdoor.sourceEdition}</b></div><div className="stat"><span>Cooling basis</span><b>{result.designConditions.selectedCoolingCondition.percentile === "percentile04" ? "0.4%" : result.designConditions.selectedCoolingCondition.percentile === "percentile1" ? "1%" : "2%"}</b></div><div className="stat"><span>Outdoor DB / MCWB</span><b>{result.designConditions.selectedCoolingCondition.dryBulbC} / {result.designConditions.selectedCoolingCondition.meanCoincidentWetBulbC} °C</b></div><div className="stat"><span>Indoor DB / RH</span><b>{result.designConditions.indoor.dryBulbC} °C / {result.designConditions.indoor.relativeHumidityPercent}%</b></div><div className="stat"><span>Outdoor RH derived from DB / MCWB</span><b>{result.designConditions.outdoor.relativeHumidityPercent.toFixed(1)}%</b></div></div>
        <div className="card"><div className="section-heading"><h3>Reference Basis</h3><span>05</span></div>{result.rooms.map((room) => { const resolved = resolvedReferenceByRoom[room.id]; return <div className="room-card" key={room.id}><div className="stat"><span>{room.id} · {room.name}</span><b>{resolved?.metadata?.referenceDatasetVersion ? `Dataset v${resolved.metadata.referenceDatasetVersion}` : "Reference data"}</b></div><div className="stat"><span>Location / activity</span><b>{resolved?.references?.location?.name || "Not selected"} · {resolved?.references?.occupantActivity?.label || "Not selected"}</b></div><div className="stat"><span>Ventilation basis</span><b>{resolved?.references?.ventilation?.label || "Not selected"}</b></div><div className="stat"><span>Source references</span><b>{resolved?.references ? Object.values(resolved.references).filter(Boolean).map((r) => r.sourceRef).join(" · ") : "None"}</b></div><ReferenceTraceabilityPanel roomName={room.name || room.id} trace={result.referenceTraceByRoom?.[room.id]} /></div>; })}</div>
...
function RoomEngineeringForm({ roomId, inputs, updateEngineering, updateEquipment, addEquipment, removeEquipment }) {
  ...
  return <div>
    ...
    <p className="form-note">Enter Rp (L/s-person) and Ra (L/s-m²) for the selected ventilation basis. Defaults are 0 until specified; the project-level dedicated-ventilation checkbox supplies a generic starter basis only when both remain 0.</p>
