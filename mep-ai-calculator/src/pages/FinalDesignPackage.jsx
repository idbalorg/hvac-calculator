...
import { createRevisionHistory, createRevision, recordRevisionApproval, getCurrentRevision, getRevisionHistorySummary, deserializeRevisionHistory } from "../engineering/report/revisionControl.js";
...
export default function FinalDesignPackage() {
  ...
  const revisionKey = `hvac-revision-history-${saved?.id || saved?.projectId || "current"}`;
  const [revisionHistory, setRevisionHistory] = useState(() => { try { const raw = localStorage.getItem(revisionKey); return raw ? deserializeRevisionHistory(raw) : null; } catch { return null; } });
  ...
  const [changeReason, setChangeReason] = useState("Initial design package");
  ...
  const [affectedDucts, setAffectedDucts] = useState("");
  ...
  const persistHistory = (history) => { setRevisionHistory(history); localStorage.setItem(revisionKey, JSON.stringify(history)); };

  const generate = (approval = null, createNewRevision = false) => {
    setError("");
    ...
    if (createNewRevision && (!revisionAuthor.trim() || !changeReason.trim())) throw new Error("Revision author and change reason are required before creating a revision.");
    ...
    const equipment = dx?.selection?.selected ? [{
      equipmentId: dx.selection.selected.indoorUnit.id || `${dx.roomId || "ROOM"}-INDOOR`, systemId: systemSummary?.systemId || "SYSTEM-01", type: dx.selection.selected.indoorUnit.type || "SPLIT_DX", manufacturer: dx.selection.selected.indoorUnit.manufacturer, model: dx.selection.selected.indoorUnit.model, refrigerant: dx.selection.selected.indoorUnit.refrigerant, capacityKw: n(dx.coverage?.selectedCapacityKW, dx.selection.selected.indoorUnit.coolingCapacityKw), requiredCapacityKw: n(dx.capacityBasis?.sizing?.requiredCapacityKW), designAirflowCfm: n(dx.requiredAirflowCfm), selectedAirflowCfm: n(dx.selection.selected.indoorUnit.airflowCfm), requiredEspPa: n(airDistribution?.integrated?.esp?.requiredFanESP_Pa, 0), selectedEspPa: n(dx.selection.selected.indoorUnit.availableEspPa, 0),
    }] : [];
    const branches = airDistribution?.integrated?.distribution?.branches || [];
    const ducts = branches.flatMap((branch) => (branch.segments || []).map((segment, index) => ({ ductId: `${branch.id || "B"}-${index + 1}`, systemId: systemSummary?.systemId || "SYSTEM-01", sectionType: "BRANCH", airflowCfm: n(segment.volumeFlowM3s) * 2118.88, widthM: n(segment.widthM, airDistribution?.inputs?.ductWidthM), heightM: n(segment.heightM), velocityMps: n(segment.velocityMps, segment.velocityMps || airDistribution?.inputs?.targetVelocityMps), pressureLossPa: n(segment.pressureLossPa) })));

    const generated = buildFinalDesignPackage({ project: saved, rooms: saved.rooms, loadResults, airsideResults, equipment, ducts, systemSummary, approval, criteria: {
      minimumCapacityMarginPercent: n(capacityMargin), maximumCapacityOversizePercent: oversize === "" ? null : n(oversize), minimumAirflowRatio: n(airflowRatio, 1), minimumEspRatio: n(espRatio, 1), distributionType, ventilationRequired: saved.result?.loads?.ventilationRequired === true, designConditionVerified: saved.result?.designConditions?.verified === true || saved.result?.designConditions?.verificationStatus === "VERIFIED", operatingConditionConfirmed: saved.result?.dxSystem?.operatingConditionConfirmed === true || saved.result?.equipmentSelection?.operatingConditionConfirmed === true, acousticCriteriaRequired: saved.result?.projectCriteria?.acousticCriteriaRequired === true, acousticCriteriaVerified: saved.result?.projectCriteria?.acousticCriteriaVerified === true, refrigerantPipingVerified: saved.result?.commissioning?.refrigerantPipingVerified === true || saved.result?.airBalanceSystem?.refrigerantPipingVerified === true, commissioningRequired: Boolean(commissioning), measuredDataAvailable: Boolean(saved.result?.airBalanceSystem?.measuredData),
    }, generatedAt: new Date().toISOString() });

    let nextHistory = revisionHistory;
    if (createNewRevision) {
      const changedAt = generated.generatedAt;
      const affected = { rooms: csv(affectedRooms), calculations: csv(affectedCalculations), equipment: csv(affectedEquipment), ducts: csv(affectedDucts) };
      nextHistory = revisionHistory
        ? createRevision({ history: revisionHistory, packageData: generated, changedBy: revisionAuthor.trim(), changedAt, reason: changeReason.trim(), affected })
        : createRevisionHistory({ packageData: generated, changedBy: revisionAuthor.trim(), changedAt, reason: changeReason.trim() });
    }
    if (approval && nextHistory) nextHistory = recordRevisionApproval({ history: nextHistory, revisionId: nextHistory.currentRevisionId, packageData: generated, approval: generated.engineeringApproval });
    if (nextHistory) persistHistory(nextHistory);
    const currentRevision = getCurrentRevision(nextHistory);
    const payload = { ...generated, revision: currentRevision ? { revisionId: currentRevision.revisionId, revisionNumber: currentRevision.revisionNumber, status: currentRevision.status, createdAt: currentRevision.createdAt, createdBy: currentRevision.createdBy, changeReason: currentRevision.changeReason, affected: currentRevision.affected, approvalInvalidated: currentRevision.approvalInvalidated } : null, sourceStages: { loads: true, airside: true, dxSystem: Boolean(dx), airDistribution: Boolean(airDistribution), airBalance: Boolean(airBalance), commissioning: Boolean(commissioning) } };
  ...
  const buildPackage = () => { try { generate(null, true); } catch (e) { setPackageResult(null); setError(e.message || "Unable to build the final design package."); } };
  const addOverride = () => { if (!overrideExceptionId || !approver.trim() || !overrideReason.trim() || !overrideEvidence.trim()) { setError("Exception, approver, override reason and evidence/reference are required."); return; } if (overrides.some((item) => item.exceptionId === overrideExceptionId)) { setError("That exception already has an override."); return; } setError(""); setOverrides((items) => [...items, { exceptionId: overrideExceptionId, approver: approver.trim(), reason: overrideReason.trim(), evidenceReference: overrideEvidence.trim(), timestamp: new Date().toISOString() }]); setOverrideExceptionId(""); setOverrideReason(""); setOverrideEvidence(""); };
  const recordApproval = () => { setError(""); if (!approver.trim() || !approvalReason.trim() || !approvalEvidence.trim()) { setError("Approver, approval reason and approval evidence/reference are required."); return; } try { generate({ status: "APPROVED", approver: approver.trim(), reason: approvalReason.trim(), evidenceReference: approvalEvidence.trim(), timestamp: new Date().toISOString(), overrides }, false); } catch (e) { setError(e.message || "Unable to record engineering approval."); } };
  ...
  const reviewRows = review ? [...review.roomChecks.flatMap((r) => [[r.roomId, "Cooling load", r.coolingLoad], [r.roomId, "Supply airflow", r.supplyAirflow], [r.roomId, "Ventilation", r.ventilation]]), ...review.equipmentChecks.flatMap((e) => [[e.equipmentId, "Capacity", e.capacity], [e.equipmentId, "Airflow", e.airflow], [e.equipmentId, "ESP", e.esp], [e.equipmentId, "Manufacturer data", e.manufacturerData]]), ...Object.entries(review.projectChecks).map(([key, check]) => ["Project", key.replace(/([A-Z])/g, " $1").replace(/^./, (c) => c.toUpperCase()), check])] : [];
  const unresolved = approval?.unresolvedExceptions || decision?.exceptions || [];
  const revisionSummary = getRevisionHistorySummary(revisionHistory);

  return <div className="container final-design-package">
    <div className="page-header"><div><p className="eyebrow">ENGINEERING WORKFLOW · STAGE 24</p><h1 className="title">Final Design Package</h1><p className="subtitle">Structured engineering schedules, standards basis, traceability, design review, exception management, approval and revision control.</p></div><span className="version-badge">Package 24.0</span></div>
    <div className="card package-controls no-print"><div className="section-heading"><h3>Package & Revision Criteria</h3><span>01</span></div>{!saved && <p className="form-note">Create and save a project first.</p>}<div className="input-grid"><Input label="Minimum capacity margin" value={capacityMargin} onChange={setCapacityMargin} placeholder="%" /><Input label="Maximum capacity oversize" value={oversize} onChange={setOversize} placeholder="%" /><Input label="Minimum airflow ratio" value={airflowRatio} onChange={setAirflowRatio} /><Input label="Minimum ESP ratio" value={espRatio} onChange={setEspRatio} /><InputText label="Revision author" value={revisionAuthor} onChange={setRevisionAuthor} /><InputText label="Change reason" value={changeReason} onChange={setChangeReason} /><InputText label="Affected rooms (comma separated)" value={affectedRooms} onChange={setAffectedRooms} /><InputText label="Affected calculations" value={affectedCalculations} onChange={setAffectedCalculations} /><InputText label="Affected equipment" value={affectedEquipment} onChange={setAffectedEquipment} /><InputText label="Affected ducts" value={affectedDucts} onChange={setAffectedDucts} /></div>{error && <p className="error-message">{error}</p>}<div className="button-row"><button onClick={buildPackage} disabled={!saved}>Generate New Design Revision</button>{packageResult && <button className="secondary-button" onClick={exportJson}>Export JSON</button>}{packageResult && <button className="secondary-button" onClick={() => window.print()}>Print / Save PDF</button>}</div><p className="form-note">Every new design generation creates a revision. If the previous revision was approved, the new revision automatically invalidates that approval.</p></div>
    {packageResult && <>
      <div className="card print-section"><div className="section-heading"><h3>Package Summary</h3><span>02</span></div><div className="stat-grid"><Stat label="Revision" value={summary.revisionId || "N/A"} /><Stat label="Revision status" value={summary.revisionStatus || "DRAFT"} /><Stat label="Validation" value={summary.validationPassed ? "PASS" : "FAIL"} /><Stat label="Engineering review" value={summary.engineeringReviewStatus} /><Stat label="Decision" value={summary.engineeringDecisionStatus} /><Stat label="Approval" value={summary.engineeringApprovalStatus} /><Stat label="Readiness" value={`${n(summary.readinessScore).toFixed(0)}/100`} /><Stat label="Exceptions" value={String(summary.exceptionCount)} /><Stat label="Rooms" value={String(summary.roomCount)} /><Stat label="Equipment" value={String(summary.equipmentCount)} /><Stat label="Cooling load" value={`${n(summary.totalCoolingLoadKw).toFixed(2)} kW`} /><Stat label="Supply airflow" value={`${n(summary.totalSupplyAirflowCfm).toFixed(0)} CFM`} /></div></div>
    ...
    <RevisionHistoryPanel history={revisionHistory} summary={revisionSummary} />
  </div>;
}
