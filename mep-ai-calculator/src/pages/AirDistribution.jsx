...
const DEFAULTS = { branches: 2, velocity: 5, width: 0.2, branchLength: 8, frictionRate: 0.5, branchK: 1, mainVelocity: 6, mainWidth: 0.3, mainLength: 6, mainK: 1, terminalDrop: 25, coilDrop: 80, filterDrop: 30, damperDrop: 20, safety: 10, tolerance: 5 };
...
export default function AirDistribution() {
  ...
  const [mainVelocity, setMainVelocity] = useState(String(DEFAULTS.mainVelocity));
  const [mainWidth, setMainWidth] = useState(String(DEFAULTS.mainWidth));
  const [mainLength, setMainLength] = useState(String(DEFAULTS.mainLength));
  const [mainK, setMainK] = useState(String(DEFAULTS.mainK));
  ...
  const calculate = () => {
    ...
    try {
      ...
      const branchAreaM2 = requiredAirflowM3s / Number(velocity) / count;
      const mainAreaM2 = requiredAirflowM3s / Number(mainVelocity);
      const branches = Array.from({ length: count }, (_, index) => ({ id: `B${index + 1}`, terminalIds: [`T${index + 1}`], airflowCfm: branchAirflowCfm, targetVelocityMps: Number(velocity), shape: "rectangular", widthM: Number(width), segments: [{ volumeFlowM3s: branchAirflowCfm / 2118.88, areaM2: branchAreaM2, lengthM: Number(branchLength), frictionRatePaPerM: Number(frictionRate), lossCoefficientK: Number(branchK) }] }));
      const mainSections = [{ id: "M1", downstreamBranchIds: branches.map((branch) => branch.id), volumeFlowM3s: requiredAirflowM3s, areaM2: mainAreaM2, lengthM: Number(mainLength), frictionRatePaPerM: Number(frictionRate), lossCoefficientK: Number(mainK) }];
      ...
      const integrated = integrateAirDistribution({ selectedEquipmentAirflowCfm: dx.selection.selected.indoorUnit.airflowCfm, availableFanEspPa: dx.selection.selected.indoorUnit.availableEspPa ?? null, requiredAirflowM3s, branches, mainSections, terminalSelection: terminal, terminalPressureDropPa: Number(terminalDrop), coilPressureDropPa: Number(coilDrop), filterPressureDropPa: Number(filterDrop), damperPressureDropPa: Number(damperDrop), espSafetyFactor: Number(safety) / 100, airflowToleranceFraction: Number(tolerance) / 100 });
      const payload = { roomId: roomResult.roomId, inputs: { branchCount: count, targetVelocityMps: Number(velocity), ductWidthM: Number(width), branchLengthM: Number(branchLength), frictionRatePaPerM: Number(frictionRate), branchLossK: Number(branchK), mainVelocityMps: Number(mainVelocity), mainWidthM: Number(mainWidth), mainLengthM: Number(mainLength), mainLossK: Number(mainK), terminalPressureDropPa: Number(terminalDrop), coilPressureDropPa: Number(coilDrop), filterPressureDropPa: Number(filterDrop), damperPressureDropPa: Number(damperDrop), espSafetyFactorPercent: Number(safety), airflowTolerancePercent: Number(tolerance) }, ...integrated };
