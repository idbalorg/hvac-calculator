...
export const runReferenceAwareEngineeringApprovalTests = () => {
  const results = [];
  const check = (id, name, condition) => { if (!condition) throw new Error(`${id} failed`); results.push({ id, name, passed: true }); };
  const readyDecision = { status: "READY_FOR_ENGINEERING_APPROVAL", readinessScore: 100, exceptions: [] };
  const reviewDecision = { status: "REVIEW_REQUIRED", readinessScore: 90, exceptions: [{ id: "REF-EXC-001", severity: "MEDIUM", scope: "R1", check: "Reference trace integrity", message: "Trace requires review" }] };
  const blockedDecision = { status: "BLOCKED", readinessScore: 75, exceptions: [{ id: "EXC-001", severity: "CRITICAL", scope: "R1", check: "Cooling load", message: "Load unresolved" }] };
  const completeApproval = { status: "APPROVED", approver: "Engineer A", reason: "Reviewed", evidenceReference: "CALC-001", timestamp: "2026-09-15T10:00:00Z" };
  ...
  const overridden = buildReferenceAwareEngineeringApproval({ engineeringDecision: reviewDecision, approval: { ...completeApproval, status: "APPROVED", overrides: [{ exceptionId: "REF-EXC-001", approver: "Engineer A", reason: "Verified against approved evidence", evidenceReference: "RFI-041", timestamp: "2026-09-15T10:00:00Z" }] } });
  ...
  const invalidOverride = buildReferenceAwareEngineeringApproval({ engineeringDecision: reviewDecision, approval: { ...completeApproval, overrides: [{ exceptionId: "REF-EXC-999", approver: "Engineer A", reason: "Unsupported", evidenceReference: "RFI-X", timestamp: "2026-09-15T10:00:00Z" }] } });
