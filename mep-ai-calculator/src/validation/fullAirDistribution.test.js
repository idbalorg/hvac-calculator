...
const branches = [
  {
    ...
    shape: "rectangular",
    widthM: 0.2,
    segments: [{ volumeFlowM3s: 250 / 2118.88, areaM2: 0.01, lengthM: 10, frictionRatePaPerM: 0.5, lossCoefficientK: 1 }],
  },
  {
    id: "B2",
    terminalIds: ["T2"],
    ...
    shape: "rectangular",
    widthM: 0.2,
    segments: [{ volumeFlowM3s: 250 / 2118.88, areaM2: 0.01, lengthM: 6, frictionRatePaPerM: 0.5, lossCoefficientK: 1 }],
  },
];
...
export const runFullAirDistributionTests = () => [
  run("AIRINT-001", "Distribution airflow conversion", () => {
    ...
    approx(result.requiredAirflowCfm, 499.25168, 1e-4);
    approx(result.airflowPerTerminalCfm, 249.62584, 1e-4);
  }),
  ...
  run("AIRINT-004", "Branch airflow is conserved", () => {
    ...
    approx(result.network.totalSupplyAirflowM3s, 500 / 2118.88, 1e-9);
  }),
  ...
  run("AIRINT-008", "Manufacturer-neutral terminal selection integrates", () => {
    const result = selectDistributionTerminal({ requiredAirflowCfm: 500, numberOfTerminals: 2, terminals: [
      { id: "T1", type: "ceiling_diffuser", minAirflowCfm: 100, maxAirflowCfm: 300, pressureDropPa: 25 },
    ] });
