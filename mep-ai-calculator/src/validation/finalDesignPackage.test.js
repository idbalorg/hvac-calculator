...
const base = {
  ...
  equipment: [{ equipmentId: "AC-01", systemId: "SYS-01", type: "SPLIT_DX", model: "DX-6000", capacityKw: 6, requiredCapacityKw: 5.5, designAirflowCfm: 424, selectedAirflowCfm: 450, requiredEspPa: 250, selectedEspPa: 300 }],
...
export const runFinalDesignPackageTests = () => {
  ...
  if (result.packageVersion !== "19.0.0") throw new Error("Package version failed");
  ...
  const directDischargePackage = buildFinalDesignPackage({ ...base, equipment: [{ ...base.equipment[0], requiredEspPa: 0, selectedEspPa: 0 }], ducts: [], systemSummary: null });
  ...
  const currentLoadShapePackage = buildFinalDesignPackage({ ...base, loadResults: [{ roomId: "R1", rawLoad: { sensibleW: 8000, latentW: 2000, totalW: 10000 }, designLoad: { sensibleW: 8800, latentW: 2200, totalW: 11000 } }] });
  ...
  if (!loadTrace || loadTrace.value !== "5.50 kW" || loadTrace.reference.includes("Fundamentals") === false) throw new Error("Cooling-load result traceability failed");
  ...
  if (!ductTrace || ductTrace.status !== "CALCULATED_VERIFY" || !ductTrace.reference.includes("SMACNA")) throw new Error("Duct result traceability failed");
  tests.push({ id: "PKG-013", name: "Result-level engineering traceability", passed: true });
